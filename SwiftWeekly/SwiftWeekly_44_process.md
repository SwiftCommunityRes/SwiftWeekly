...
**回复**
Swift 运行时会传递到 malloc 和 free，因此您实际上只是在此处分析系统分配器的内存使用模式。 不同的 malloc 实现可能会以您喜欢的方式重复使用内存。
...
然而，LSG 对提议的名称感到满意。 RangeSet 被认为是更通用的结构，并且像index(of:) 这样的方法名称被认为是合适的，因为 RangeSet<IndexType> 代表了比单独的集合索引集更广泛的概念。
