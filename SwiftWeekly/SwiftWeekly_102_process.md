...
Slava Pestov 介绍了 **Swift 6.4** 开发快照中已落地的一组 **类型检查器** 性能改进。核心变化包括 **disjunction pruning** 和更精确的 **binding inference**：前者在重载解析时不只区分“优先/非优先”，还会直接禁用“不可能成功”的候选；后者从收集候选绑定，转向分析类型变量可能取值的域，从而更早发现唯一解或死路。
...
讨论亮点在于，这些优化还让编译器可以移除一些更窄、更硬编码的旧启发式逻辑，并把更多曾经超时的表达式纳入测试。社区追问是否会像 Swift 6.3 的可选类型改进那样带来正确性回归。简评：这类工作不如新语法显眼，但会直接影响日常编码体验，尤其能减少“表达式过复杂无法在合理时间内类型检查”的痛点。

### 2、赋值到 existential 时应使用 any 还是 some
...
这个问题聚焦在 **some** 与 **any** 的边界：如果初始化器最终只是把值存入 `any Hashable`，参数写成 `some Hashable` 和 `any Hashable` 是否等价。示例代码如下：
...
讨论亮点是 API 演进取舍：如果未来可能不再存储 type-erased 值，`some` 会保留转向泛型实现的空间；但若设计目标就是立刻存入 existential，`any` 更直接。简评：这是一处少见的“优先 existential 参数”的场景，性能敏感代码尤其值得注意。
...
这里的关键点是 **SwiftSyntax** 宏拿到的是已经解析后的语法树。字符串插值只有在字符串字面量内部才有意义，例如 `"\(value)"`；而 `\("Helo")` 本身不是一个字符串字面量表达式，因此第一层 `StringLiteralExprSyntax` 匹配就会失败。若要测试插值分支，应传入真正的字符串字面量，并让 `representedLiteralValue` 因包含插值而返回 `nil`。

讨论亮点在于，宏测试中 raw string 的写法容易让人误以为外层测试字符串里的 `\(...)` 会变成被测代码里的字符串插值。简评：诊断分层要先确认语法节点类型，再检查字面量内容；测试用例最好覆盖“非字符串表达式”和“含插值的字符串字面量”两个独立路径。

### 4、切换 Git 分支后 Xcode 仍可能丢失 Swift Package 产品
...
动机和影响都很现实：复杂项目通常依赖多个 **SPM** 包，分支切换会改变 `Package.resolved`、包版本或本地包布局；如果 Xcode 没有及时刷新依赖图，构建就会被阻断。后续回复中，Kai Engelhardt 和 Alexei 也确认这是团队日常开发中的长期痛点，并希望 Xcode 与 SPM 集成能被提升优先级。

讨论中常见临时处理包括重启 Xcode、清理 DerivedData、重置包缓存、重新解析包，严重时甚至需要重新 clone。简评：这不是语言层面的 Swift 问题，却会直接影响 Swift 开发效率；在大型工程中，包解析缓存和 IDE 状态一致性仍是需要重点打磨的基础体验。

### 5、SwiftOS：用 Embedded Swift 从零构建内核和用户态
...
讨论亮点包括是否能移植到 Raspberry Pi、Swift 是否需要内联汇编、以及真实云主机带来的 ACPI、GICv3、PCIe、virtio-net 等差异。后续作者还补充 OpenSSL 在 Hetzner VM 上缺少熵源的问题，并用内核 jitter entropy 通过 `SYS_RANDOM` 解决。简评：SwiftOS 展示了 **Embedded Swift** 已经能触达非常底层的系统编程场景，也暴露出 freestanding 工具链、链接器和硬件抽象仍需要继续完善。
